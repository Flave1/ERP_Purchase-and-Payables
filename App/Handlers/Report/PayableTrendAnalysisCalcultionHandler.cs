...
{
    public class PayableDaysTrendAnalysis: IRequest<PayableDaysTrendAnalysisResp>
    {
        public class PayableDaysTrendAnalysisHandler : IRequestHandler<PayableDaysTrendAnalysis, PayableDaysTrendAnalysisResp>
        {
            private readonly DataContext _dataContext;
            private readonly IFinanceServerRequest _financeServer;
            public PayableDaysTrendAnalysisHandler(DataContext dataContext, IFinanceServerRequest financeServer)
            {
                _financeServer = financeServer;
                _dataContext = dataContext;
            }
            public async Task<PayableDaysTrendAnalysisResp> Handle(PayableDaysTrendAnalysis request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    foreach(var group in byYearGroup)
                    {
                        ...
                        var total = group.Where(t => debitGls.Contains(t.SubGL)).ToList().Sum(r => r.DebitAmount) / group.Where(t => creditGls.Contains(t.SubGL)).ToList().OrderBy(t => t.TransactionId).FirstOrDefault().RunningBal * 365;
                        Labels.Add(Year);
                        dataValue.Add(total??0);
                    } 
 
                    
                    var dataset = new PayableDays
                    {
                        Label = "Payable Trend Analysis",
                        BorderColor = "",
                        Data = dataValue.ToArray(),
                        Fill = false,
                    };
